...
const createSession = async (req, res) => {
  const { participants, votes, majorityVote, averageVote, createdAt } = req.body;
  try {
...
const updateSession = async (req, res) => {
  ...
  const voteList = updatedVoteList.filter((user) => user.vote != "0" && user.vote != "?").map((user) => (user.vote))
